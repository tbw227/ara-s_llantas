    ...
    try {
      ...
      if (existingSubscriber) {
        ...
        if (existingSubscriber.status === 'active') {
          ...
          await db('newsletter_subscribers').where('email', normalizedEmail).update({
            status: 'active',
            updated_at: new Date(),
          });
      ...
      const existingInMemory = newsletterSubscribers.find((s) => s.email === normalizedEmail);
...
router.post('/newsletter/unsubscribe', async (req, res) => {
  try {
    ...
    try {
      ...
      const updated = await db('newsletter_subscribers').where('email', normalizedEmail).update({
        status: 'unsubscribed',
        updated_at: new Date(),
      });
      ...
      // Fallback to in-memory storage
      const subscriber = newsletterSubscribers.find((s) => s.email === normalizedEmail);
